...
export class MinIOService implements OnModuleInit {
	...
	async uploadFile(bucketName: string, fileName: string, file: Buffer, mimeType: string): Promise<string> {
		...
		await this.client.putObject(bucketName, fileName, file, file.length, { 'Content-Type': mimeType });
	...
	async getFileUrl(bucketName: string, fileName: string): Promise<string> {
		try {
			...
			this.logger.warn(`Não foi possível gerar URL para ${fileName} no bucket ${bucketName}: ${error.message}`);
			return null;
		}
	}
